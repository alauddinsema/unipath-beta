...
function registerValidSW(swUrl: string, config?: Config) {
  navigator.serviceWorker
    .register(swUrl)
    .then((registration) => {
      // Don't check for updates too frequently to prevent refreshes on tab switching
      // Only check for updates once per hour
      const lastUpdateCheck = parseInt(localStorage.getItem('lastUpdateCheck') || '0');
      const now = Date.now();
      const ONE_HOUR = 60 * 60 * 1000;
      
      if (now - lastUpdateCheck > ONE_HOUR) {

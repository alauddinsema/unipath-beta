...
export function useMistralChat() {
  ...
  const getMistralResponse = async (userMessage: string): Promise<string> => {
    try {
      console.log('Calling Mistral API with message:', userMessage);
      ...
      console.log('Received Mistral response successfully');
      return data.content;
    } catch (error) {
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      const aiResponse = await getMistralResponse(messageToSend);
